...
(defun plot-list (l)
  ...
  (let ((fname "clisp-one-var-plot.dat")
				(script (concatenate 'string (warp:bin-dir) "clisp_gplot_one_var.sh")) )
    (warp:list-to-file l fname)
		(sleep 0.5)
...
(defun plot-xy (a b)
  ...
  (let ((fname "clisp-xy-plot.dat")
				(script (concatenate 'string (warp:bin-dir) "clisp_gplot_xy.sh")))
...
(defun plot-scatterplot (a b)
  ...
  (let ((fname "clisp-scatterplot.dat")
				(script (concatenate 'string (warp:bin-dir) "clisp_gplot_scatterplot.sh")))
    (with-open-file (f fname :direction :output)
      (mapcar (lambda (x y)
                (format f "~% ~a ~a" x y)) a b))
		(sleep 0.5)		
